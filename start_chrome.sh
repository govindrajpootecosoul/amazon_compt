#!/usr/bin/env bash
# Opens Chrome with debug port 9222 for the Walmart scraper (Mac/Linux).
...
echo "Starting Chrome on port $PORT for Walmart..."
...
echo
echo "Wait ~5 seconds, then check Walmart homepage."
echo "1) Complete Press & Hold captcha if shown"
echo "2) Keep this Chrome window open (do not quit Chrome)"
echo "3) Mac screen lock is OK — do NOT Quit Chrome / Force Quit"
echo "4) Run:  python3 scraper.py"
echo
